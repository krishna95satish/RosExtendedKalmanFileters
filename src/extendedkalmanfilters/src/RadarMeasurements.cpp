...
void rosBagCreator :: read() {
  ...
  while (getline(finput, line)) {
    ...
    if (sensor_type == "L") {
      ...
      std::string s_type;
      float x_m, y_m, t, x_gt, y_gt, vx_gt, vy_gt;
      ...
      std::string s_type;
      float rho, phi, rhodot, t, x_gt, y_gt, vx_gt, vy_gt;
...
void rosBagCreator :: radarWrite(std::string& s_type, float rho, float phi, float rhodot, float t, float x_gt, float y_gt, float vx_gt, float vy_gt) {
...
void rosBagCreator :: lidarWrite(std::string& s_type, float x_m, float y_m, float t, float x_gt, float y_gt, float vx_gt, float vy_gt) {
